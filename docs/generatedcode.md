...
You can double-click on this `.cs` file to see the contents. If your editor doesn't have such a view, or you'd simply
rather have the files on disk, add `<EmitCompilerGeneratedFiles>true</EmitCompilerGeneratedFiles>` to your project and
build: the same code is written to `obj/<configuration>/<target-framework>/generated/`.

We'll use the code from [Getting Started](/gettingstarted) to illustrate.
